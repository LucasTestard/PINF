...
function mkEnteteTabUser()
{
	echo "\t<tr>\n";
		echo "\t\t<th scope='col' id='celltab'>Nom</th>\n";
		echo "\t\t<th scope='col' id='celltab'>Prénom</th>\n";
		echo "\t\t<th scope='col' id='celltab'>Login</th>\n";
		echo "\t\t<th scope='col' id='celltab'>Mot de passe</th>\n";
		echo "\t\t<th scope='col' id='celltab'>Rôle</th>\n";
	echo "\t</tr>\n";
}

function mkLigneTabUser($DataUser,$i)
{
	
	echo "\t<tr>\n";
		/*echo "\t\t<td>$i</td>\n";*/
		foreach($DataUser as $cle => $val)	
		{
			echo "\t\t<td id='celltab'>$val</td>\n";
		}
	echo "\t</tr>\n";	
}

// Exemple d'appel :  mkTable($users,array('pseudo', 'couleur', 'connecte'));	
function mkTabUser($tabData)
{

	echo "<table id=\"TabUSer\" class='table table-bordered table-striped mb-0'>\n";
	$i=1;
	mkEnteteTabUser();
	foreach ($tabData as $DataUser)	
	{
		// afficher une ligne de données avec les valeurs, à chaque itération
		mkLigneTabUser($DataUser,$i);
		$i=$i+1;
	}
	echo "</table>\n";
}
...
function mkEnteteTabVehicule()
{
	echo "\t<tr>\n";
		echo "\t\t<th scope='col' id='celltab'>Immatriculation</th>\n";
		echo "\t\t<th scope='col' id='celltab'>Type de véhicule</th>\n";
		echo "\t\t<th scope='col' id='celltab'>Kilométrage</th>\n";
	echo "\t</tr>\n";
}

function mkLigneTabVehicule($DataVehicule,$i)
{
	
	echo "\t<tr>\n";
		/*echo "\t\t<td>$i</td>\n";*/
		foreach($DataVehicule as $cle => $val)	
		{
			echo "\t\t<td id='celltab'>$val</td>\n";
		}
	echo "\t</tr>\n";	
...
// Exemple d'appel :  mkTable($users,array('pseudo', 'couleur', 'connecte'));	
function mkTabVehicule($tabData)
{

	echo "<table id=\"TabVehicule\" class='table table-bordered table-striped mb-0'>\n";
	$i=1;
	mkEnteteTabVehicule();
	foreach ($tabData as $DataVehicule)	
	{
		// afficher une ligne de données avec les valeurs, à chaque itération
		mkLigneTabVehicule($DataVehicule,$i);
		$i=$i+1;
	}
	echo "</table>\n";
}
